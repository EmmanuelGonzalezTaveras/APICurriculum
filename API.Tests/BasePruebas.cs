using API.DTOs;
using API.Tests.PruebasUnitarias;
using AutoMapper;
using Entidades;
...
namespace API.Tests
{
    ...
    {
        protected Guid usuarioPorDefectoId = new Guid("9722b56a-77ea-4e41-941d-e319b6eb3712");
        ...
        protected ControllerContext ConstruirControllerContext()
        {
            var usuario = new ClaimsPrincipal(new ClaimsIdentity(new Claim[]
            {
                ...
                new Claim(ClaimTypes.NameIdentifier, usuarioPorDefectoId.ToString())
            }));
